...
public class CamelCardsDeluxe {
    public static void main(String args[]) throws Exception {
        FileInputStream fis = new FileInputStream("./input.txt");
        Scanner sc = new Scanner(fis);

        final String CARD_ORDER = "J23456789TQKA";

        List<HandDeluxe> hands = new LinkedList<HandDeluxe>();
        ...
        while (sc.hasNextLine()) {
            ...
            HandDeluxe hand = new HandDeluxe(bid, cards);
...
class HandDeluxe {
    ...
    public HandDeluxe(int bid, int[] cards) {
        this.bid = bid;
        this.cards = cards;
        ...
        int n_jokers = (card_counts.get(0) == null) ? 0 : card_counts.remove(0);
        int max_count_same = 5;
        if (!card_counts.isEmpty()) {
            int max_count_same_nonjoker = Collections.max(card_counts.values());
            for (int key : card_counts.keySet()) {
                if (card_counts.get(key) == max_count_same_nonjoker) {
                    card_counts.put(key, max_count_same_nonjoker + n_jokers);
                    break;
                }
            }
            max_count_same = max_count_same_nonjoker + n_jokers;
        }
    ...
    public boolean stronger_than(HandDeluxe hand) {
