...
int main(int argc, char *argv[]) {
  ...
  int median = 0;
  if (SUB_COUNT % 2 == 0) {
    // if its an even amount of values take the mean between the two middle
    // values
    median = (int)(subs[(int)((SUB_COUNT - 1) / 2)] +
                   subs[(int)((SUB_COUNT - 1) / 2 + 1)]) /
             2;
  } else {
  ...
  if (printf("The ideal position is at %i. The total fuel consumption of the "
             "swarm to get there is %i units.\n",
             median, totalFuel) < 0) {
    perror("printf");
    exit(EXIT_FAILURE);
  }
