...
static void flash(int map[ROW_COUNT][COL_COUNT], int row, int col) {
  // set the cell value to 100 to signify it has been flashed and should not be
  // flashed a second time in this step.
  map[row][col] = 100;
  // increase the energy level of all 8 surrounding octopuses by 1.
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      if (i != 0 || j != 0) { // skip the middle
        if (row + i != -1 && col + j != -1 && row + i != ROW_COUNT &&
            col + j != COL_COUNT) {
          map[row + i][col + j]++;
        }
      }
  ...
  // if this raised their individual energy level above 9, it also needs to
  // flash.
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      if (i != 0 || j != 0) { // skip the middle
        if (row + i != -1 && col + j != -1 && row + i != ROW_COUNT &&
            col + j != COL_COUNT && map[row + i][col + j] > 9 &&
            map[row + i][col + j] <
                100) { // if it is >=100, its flash has been simulated already
