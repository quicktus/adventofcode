...
fn main() {
    ...
    for pattern in patterns {
        let n_constructions: u32 = count_constructions(pattern, &towels, &mut cache);
        if n_constructions > 0 {
            result_1 += 1;
            result_2 += n_constructions;
        }
    }
...
fn count_constructions(pattern: &str, towels: &Vec<&str>, cache: &mut HashMap<String, u32>) -> u32 {
    if pattern.is_empty() {
        return 1;
    }
