...
const SYMMETRY_THRESHOLD: f64 = 0.1;
...
fn main() {
    ...
    loop {
        ...
        let n_total = filled_squares.len() as f64;
        let mut n_sym = 0.0;
        for fs in &filled_squares {
            if filled_squares.binary_search(&rev(*fs)).is_ok() {
                n_sym += 1.0;
            }
        }

        if n_sym >= n_total * SYMMETRY_THRESHOLD {
